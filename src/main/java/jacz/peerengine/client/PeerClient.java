...
import jacz.peerengine.ErrorControl;
import jacz.peerengine.PeerID;
import jacz.peerengine.client.connection.*;
...
public class PeerClient {
    ...
    /**
     * Object for synchronizing lists (utility for the user)
     */
    ...
    public PeerClient(
        ...
        this(peerClientData, peerClientAction, globalDownloadStatistics, globalUploadStatistics, peerStatistics, peerRelations, customFSMs, null, false);
    }
     ...
     * @param peerRelations    relations with other peers. This object will be updated during the execution, as we add or reject peers
     * @param customFSMs       list of factories for building custom FSMs, each with a different name
     * @param listContainer    ListContainer for sharing lists with peers using the provided list synchronization methods (optional, null if not used)
     */
    public PeerClient(
            PeerClientData peerClientData,
            PeerClientAction peerClientAction,
            ...
            boolean allowSynchronizingBetweenNonFriendPeers) {
        ...
        if (listContainer != null) {
            listSynchronizer = new ListSynchronizer(this, listContainer, peerClientData.getOwnPeerID(), allowSynchronizingBetweenNonFriendPeers);
        }
    ...
    public synchronized void addFriendPeer(final PeerID peerID) {
        ...
        if (connectionStatus != null && connectionStatus == ConnectionStatus.NOT_FRIEND) {
     ...
     * @throws IndexOutOfBoundsException the given PeerID does not correspond to a connected peer
     */
    public synchronized <T> boolean registerCustomFSM(PeerID peerID, PeerFSMAction<T> peerFSMAction, String serverFSMName) throws IndexOutOfBoundsException {
        ...
        if (connectedPeers.isConnectedPeer(peerID)) {
            ...
            throw new IndexOutOfBoundsException("The given PeerID does not correspond to any connected peer: " + peerID);
        }
    }
     ...
     * @throws IndexOutOfBoundsException the given PeerID does not correspond to a connected peer
     */
    public synchronized <T> boolean registerTimedCustomFSM(PeerID peerID, PeerTimedFSMAction<T> peerTimedFSMAction, String serverFSMName, long timeoutMillis) throws IndexOutOfBoundsException {
        if (connectedPeers.isConnectedPeer(peerID)) {
            Byte assignedChannel = connectedPeers.requestChannel(peerID);
            if (assignedChannel == null) {
                return false;
    ...
    public void sendChatMessage(PeerID peerID, String message) {
        connectedPeersMessenger.sendChatRequest(peerID, message);
    }
     ...
     * @param message string message received
     */
    synchronized void newChatMessageReceived(final PeerID peerID, final String message) {
        if (connectedPeers.isConnectedPeer(peerID)) {
            sequentialTaskExecutor.executeTask(new ParallelTask() {
                @Override
                public void performTask() {
                    peerClientAction.newChatMessage(peerID, message);
                }
            });
        }
    }
     ...
     * of him (CORRECT), he is not a friend of us but we are a friend of him (NOT_FRIEND) or he is our friend but we are waiting for
